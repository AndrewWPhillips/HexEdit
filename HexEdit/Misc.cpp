...
LONGLONG DeviceSize(LPCTSTR filename) // xxx get rid of this - use SpecialList::total_size() instead)
{
	ASSERT(filename != NULL && ::IsDevice(filename));
	LONGLONG retval = -1;
    if (_tcslen(filename) == 6)
	{
		if (filename[5] != ':')
			return -1;
...
#if 0 // returning sector size no longer supported
		// Get sector size
		if (pSectorSize != NULL)
		{
		...
		if (!::GetDiskFreeSpaceEx(vol, &dummy1, &total_bytes, &dummy2))  // xxx need to handle no medium present
		{
			...
			VERIFY(::GetDiskFreeSpace(filename,				// This fails if called after device opened under XP
									...
									&NumberOfFreeClusters,
									&TotalNumberOfClusters));
			retval = (LONGLONG)SectorSize * SectorsPerCluster * (LONGLONG)TotalNumberOfClusters;
		}
		else
	...
	else if (_tcsncmp(filename, _T("\\\\.\\CdRom"), 9) == 0)
	{
		...
		for (int ii = 0; ii < 26; ++ii)
		{
			vol[0] = 'A' + (char)ii;
			vol[2] = '\\';
			if ((dbits & (1<<ii)) != 0 && ::GetDriveType(vol) == DRIVE_CDROM)
			{
				char buf[128];
				buf[0] = '\0';
				vol[2] = '\0';     // QueryDosDevice does not like the backslash
				::QueryDosDevice(vol, buf, sizeof(buf));  // eg \device\cdrom0
	...
	else
	{
		ASSERT(_tcsncmp(filename, _T("\\\\.\\PhysicalDrive"), 17) == 0 ||
			   _tcsncmp(filename, _T("\\\\.\\Floppy"), 10) == 0 ||
			   _tcsncmp(filename, _T("\\\\.\\CdRom"), 9) == 0);
        ...
        HANDLE fh;
		VERIFY((*pfOpenFile)(&fh, FILE_READ_DATA|SYNCHRONIZE, &oa, &iosb,
            FILE_SHARE_READ|FILE_SHARE_WRITE, FILE_NON_DIRECTORY_FILE|FILE_SYNCHRONOUS_IO_ALERT) == STATUS_SUCCESS);

        // First check if media present (else we can't determine size)
		if ((*pfDeviceIoControlFile)(fh, 0, 0, 0, &iosb, IOCTL_STORAGE_CHECK_VERIFY, 0, 0, 0, 0) == STATUS_SUCCESS)
		{
			if (theApp.is_xp_ && _tcsncmp(filename, _T("\\\\.\\Floppy"), 10) != 0)
			{
				GET_LENGTH_INFORMATION gle;
				VERIFY((*pfDeviceIoControlFile)(fh, 0, 0, 0, &iosb, IOCTL_DISK_GET_LENGTH_INFO, 0, 0, &gle, sizeof(gle)) == STATUS_SUCCESS);
				retval = gle.Length.QuadPart;
			}
			else
			{
				DISK_GEOMETRY dg;
				//if (_tcsncmp(filename, _T("\\\\.\\CdRom"), 9) == 0)
				//	VERIFY((*pfDeviceIoControlFile)(fh, 0, 0, 0, &iosb, IOCTL_CDROM_GET_DRIVE_GEOMETRY, 0, 0, &dg, sizeof(dg)) == STATUS_SUCCESS);
				//else
					VERIFY((*pfDeviceIoControlFile)(fh, 0, 0, 0, &iosb, IOCTL_DISK_GET_DRIVE_GEOMETRY, 0, 0, &dg, sizeof(dg)) == STATUS_SUCCESS);

				retval = dg.Cylinders.QuadPart * dg.TracksPerCylinder *
